...
export default function Dashboard({ 
  ...
  // Dynamic Year list generator
  const yearsList = React.useMemo(() => {
    const list = new Set<string>();
    shifts.forEach(s => {
      if (s.date) list.add(s.date.substring(0, 4));
    });
    expenses.forEach(e => {
      if (e.date) list.add(e.date.substring(0, 4));
    });
  ...
  const [selectedYear, setSelectedYear] = React.useState<string>(() => yearsList[0] || new Date().getFullYear().toString());
  ...
  const yearFixedAluguelSeguros = React.useMemo(() => {
    return expenses
      .filter(e => e.date && e.date.startsWith(selectedYear) && (e.category === 'Aluguel' || e.category === 'Financiamento' || e.category === 'Seguro'))
      .reduce((acc, curr) => acc + curr.value, 0);
  }, [expenses, selectedYear]);

  const yearFixedCelularInternet = React.useMemo(() => {
    return expenses
      .filter(e => e.date && e.date.startsWith(selectedYear) && e.category === 'Celular/Internet')
      .reduce((acc, curr) => acc + curr.value, 0);
  }, [expenses, selectedYear]);

  const yearFixedOutrasAdm = React.useMemo(() => {
    return expenses
      .filter(e => e.date && e.date.startsWith(selectedYear) && (e.category === 'IPVA/Impostos' || e.category === 'Outros'))
      .reduce((acc, curr) => acc + curr.value, 0);
  }, [expenses, selectedYear]);
  ...
  const annualGoalTarget = profile.annualGoal || (profile.monthlyGoal * 12 || 54000);
  const annualProgressPercent = Math.min(100, Math.max(0, (yearNet / annualGoalTarget) * 100));

  // Platform chart data formatting based on selectedYear
  const platformChartData = React.useMemo(() => {
    return [
      { name: 'Uber App', value: yearUber, color: '#09090b' },
      { name: '99 App', value: year99, color: '#facc15' },
      { name: 'InDrive', value: yearIndrive, color: '#14b8a6' },
      { name: 'Particular', value: yearPrivate, color: '#2563eb' },
      { name: 'Gorjetas & Tips', value: yearOther, color: '#f97316' }
    ].filter(item => item.value > 0);
  }, [yearUber, year99, yearIndrive, yearPrivate, yearOther]);

  // Expense Breakdown data formatting based on selectedYear
  const expenseChartData = React.useMemo(() => {
    return [
      { name: 'Combustível (GNV/L)', value: yearShiftFuel, color: '#f97316' },
      { name: 'Lanches/Alimentação', value: yearShiftFood, color: '#fbbf24' },
      { name: 'Tag Sem Parar/Limpeza', value: yearShiftOther, color: '#06b6d4' },
      { name: 'Parcl/Aluguel/Seguros', value: yearFixedAluguelSeguros, color: '#8b5cf6' },
      { name: 'Internet/Plano Celular', value: yearFixedCelularInternet, color: '#ec4899' },
      { name: 'Outras Adm.', value: yearFixedOutrasAdm, color: '#64748b' }
    ].filter(item => item.value > 0);
  }, [yearShiftFuel, yearShiftFood, yearShiftOther, yearFixedAluguelSeguros, yearFixedCelularInternet, yearFixedOutrasAdm]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 rounded-2xl border border-slate-200 shadow-xs gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-semibold">Análise consolidada e evolução financeira anual.</p>
          </div>
        </div>
        
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <span className="text-xs font-bold text-slate-500 whitespace-nowrap">Ano de Referência:</span>
          <select
            value={selectedYear}
      ...
      {/* 2. CORE FINANCIAL BALANCES HIGHLIGHTS (ANNUAL ONLY AS REQUESTED) */}
      <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl text-white shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          {/* Core Register Annual Goal Target */}
          <div className="p-4 rounded-xl bg-slate-850 border border-slate-800/70 flex flex-col justify-between">
            <div>
              <span className="text-[9px] uppercase font-mono font-extrabold text-slate-400 tracking-wider block">Meta Líquida Cadastrada</span>
              <p className="text-lg font-black text-blue-300 font-mono mt-1">R$ {annualGoalTarget.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
            </div>
            <span className="text-[9px] text-slate-500 font-bold mt-1 inline-block border-t border-slate-800/40 pt-1 font-mono">Atingido: {annualProgressPercent.toFixed(1)}%</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-slate-900 text-white rounded-2xl p-6 border border-slate-800 flex flex-col justify-between space-y-4">
          <div className="space-y-4">
            ...
            <h4 className="text-lg font-black tracking-tight leading-snug">Meta Anual Líquida de Lucro</h4>
            ...
            <div className="space-y-2 pt-2">
              <div className="flex justify-between items-baseline text-xs font-bold font-mono">
                ...
                <span className="text-blue-400 text-base">R$ {yearNet.toLocaleString('pt-BR', { maximumFractionDigits: 0 })}</span>
              </div>
              {/* Horizontal line indicator */}
              <div className="w-full bg-slate-800 rounded-full h-3 overflow-hidden border border-slate-700">
                <div className="bg-blue-500 h-full rounded-full transition-all duration-300" style={{ width: `${annualProgressPercent}%` }} />
              </div>
              <div className="flex justify-between text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                <span>{annualProgressPercent.toFixed(1)}% Completo</span>
                <span>Alvo: R$ {annualGoalTarget.toLocaleString('pt-BR', { maximumFractionDigits: 0 })}</span>
              </div>
            </div>
          </div>
        ...
        {/* TIME GRAPH LINE BALANÇO (ANNUAL ONLY) */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-2">
        ...
        {/* PLATFORMS PIE CHART */}
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-4">
          <span className="text-[10px] uppercase font-mono font-black text-slate-400 tracking-wider block">Ganhos por Plataforma de Trabalho (no Ano)</span>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="h-44 w-1/2 min-w-[160px] relative">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center select-none pointer-events-none">
                <span className="text-[9px] uppercase font-bold text-slate-450 block">Bruto Ano</span>
                <span className="text-sm font-black text-slate-900 font-mono">R$ {yearGross.toFixed(0)}</span>
              </div>
            </div>
            ...
            <div className="flex-1 space-y-2 text-xs font-semibold text-slate-650 w-full">
              {platformChartData.map((item, idx) => (
                <div key={idx} className="flex justify-between items-center text-[11px]">
                  ...
                  <span className="font-mono text-slate-900 font-bold">R$ {item.value.toFixed(0)} ({((item.value / (yearGross || 1))*100).toFixed(0)}%)</span>
                </div>
              ))}
              {platformChartData.length === 0 && (
                <p className="text-[10px] text-slate-405 italic">Nenhum faturamento registrado neste ano.</p>
              )}
            </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-205 shadow-xs space-y-4">
          <span className="text-[10px] uppercase font-mono font-black text-slate-400 tracking-wider block">Quadro de Distribuição de Despesas (no Ano)</span>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="h-44 w-1/2 min-w-[160px] relative">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center select-none pointer-events-none">
                <span className="text-[9px] uppercase font-bold text-slate-450 block">Custos Ano</span>
                <span className="text-sm font-black text-red-650 font-mono">R$ {yearConsolidatedExpenses.toFixed(0)}</span>
              </div>
            </div>
            ...
            <div className="flex-1 space-y-2 text-xs font-semibold text-slate-650 w-full">
              {expenseChartData.map((item, idx) => (
                <div key={idx} className="flex justify-between items-center text-[11px]">
                  ...
                  <span className="font-mono text-red-650 font-bold">R$ {item.value.toFixed(0)} ({((item.value / (yearConsolidatedExpenses || 1))*100).toFixed(0)}%)</span>
                </div>
              ))}
              {expenseChartData.length === 0 && (
                <p className="text-[10px] text-slate-405 italic">Nenhuma despesa ou custo registrado neste ano.</p>
              )}
            </div>
